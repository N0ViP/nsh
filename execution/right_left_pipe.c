...
/*   Updated: 2025/08/22 06:55:30 by ahoummad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
pid_t	fork_left_pipe(int pipefd[2], t_tree *branch)
{
	...
	if (pid == 0)
	{
		...
		exit_shell(execution_mode(branch->u_data.branch.left, NO_FORK_MODE));
	}
	return (pid);
}
...
pid_t	fork_right_pipe(int pipefd[2], t_tree *branch)
{
	...
	if (pid == 0)
	{
		...
		close_and_remove(pipefd[0]);
		exit_shell(execution_mode(branch->u_data.branch.right, NO_FORK_MODE));
	}
	return (pid);
}
